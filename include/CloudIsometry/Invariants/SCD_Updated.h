#ifndef SCD_UPDATED_H
#define SCD_UPDATED_H


class SCD_Updated
{
public:
    SCD_Updated():constC(0.43) {}
    SCD_Updated(double dt):constC(dt) {}
    virtual ~SCD_Updated() {}
    ...
    void ConstructCloudInvariantCombo(std::pair<arma::mat, std::vector<std::vector<bool>>> & cloudA, std::vector<std::vector<int>> & combos,
                                      std::vector<std::tuple<double, arma::mat, arma::mat>> & finalCollection)
    {
        for (int i = 0; i < combos.size(); i++)
        {
            ...
            this->theOthetMatrix(combos[i],counterCombos,cloudA.first,notSelectedpoints);
             arma::mat ocdCloud;
            double sign = ComputeSign(selectedPoints);
    ...
    double ComputeSign(arma::mat & selectedPoints)
    {
        ...
        return answer/this->constC * strengthh;
